    ...
    function drop(event:React.DragEvent){
       if(!allowed){ 
        ...
        if(data){
            ...
            socket.emit("move",{fromX,fromY,toX:posX,toY:posY});
        }
        } }
    }
 
    return( <Button style={{backgroundColor:bg}} draggable={Boolean(pvalue&&allowed)} onDragOver={dragOver} onDrop={drop} onDragStart={dragStart} className={"opacity-50 shadow-md min-w-20 min-h-20 rounded-none overflow-hidden "}  >
            <p className='bg-transparent flex items-center justify-center text-[4vh] w-full h-full rounded-md' style={{ color: identity }} dangerouslySetInnerHTML={{ __html: value }}></p>
        </Button>
        )
