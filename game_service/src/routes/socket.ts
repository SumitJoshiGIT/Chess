...
export const setupSocketIO = (server: any) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-matched-game', async ({ gameId }) => {
      if (gameId) {
        socket.join(gameId);
        console.log(`[${new Date().toISOString()}] User ${userId} joined matched game room: ${gameId}`);
        try {
          ...
          if (gameStateStr) {
            ...
            console.log(`[${new Date().toISOString()}] Emitted \'game-joined\' to ${userId} for matched game ${gameId}`);
          } else {
            console.warn(`[${new Date().toISOString()}] No game state found for matched game ${gameId} when ${userId} tried to join.`);
            socket.emit('error', { message: `Game state for ${gameId} not found.` });
          }
        } catch (err) {
          console.error(`[${new Date().toISOString()}] Error sending game state to ${userId} for matched game ${gameId}:`, err);
          socket.emit('error', { message: 'Error retrieving game state.' });
        }
      } else {
        console.log(`[${new Date().toISOString()}] User ${userId} tried to join a matched game without gameId`);
        socket.emit('error', { message: 'gameId not provided for join-matched-game.' });
      }
    });

    socket.on('join-game', async ({ gameId }) => {
      if (!gameId) {
        console.log(`[${new Date().toISOString()}] User ${userId} tried to join a game without gameId (direct join)`);
        socket.emit('error', { message: 'gameId not provided for join-game.' });
        return;
      }
      try {
        console.log(`[${new Date().toISOString()}] User ${userId} attempting to directly join game: ${gameId}`);
        const gameStateStr = await redisClient.get(`game:${gameId}:state`);
        if (gameStateStr) {
          const gameState = JSON.parse(gameStateStr);
          socket.join(gameId);
          console.log(`[${new Date().toISOString()}] User ${userId} successfully joined room for game: ${gameId}`);
          
          socket.emit('game-joined', {
            gameId,
            gameState
          });
          console.log(`[${new Date().toISOString()}] Emitted \'game-joined\' to ${userId} for direct join to game ${gameId}`);
        } else {
          console.warn(`[${new Date().toISOString()}] Game not found: ${gameId} when ${userId} tried to directly join.`);
          socket.emit('error', { message: `Game ${gameId} not found or has expired.` });
        }
      } catch (error) {
        console.error(`[${new Date().toISOString()}] Error during direct join to game ${gameId} for user ${userId}:`, error);
        socket.emit('error', { message: 'Failed to join game due to server error.' });
      }
    });
    ...
    socket.on('join-matched-game', async ({ gameId }) => {
      if (gameId) {
        ...
        // Emit current game state and clock to the joining player
        try {
          const gameStateStr = await redisClient.get(`game:${gameId}:state`);
          if (gameStateStr) {
            const gameState = JSON.parse(gameStateStr);
          ...
          console.error(`[${new Date().toISOString()}] Error sending game state to ${userId} for game ${gameId}:`, err);
        }
      } else {
    ...
    // Handle chess moves using PGN (SAN)
    socket.on('make-move', async ({ gameId, pgnMove, promotion }) => {
      try {
        console.log(`[${new Date().toISOString()}] User ${userId} making move in game ${gameId}: ${pgnMove}${promotion ? ` with promotion=${promotion}` : ''}`);
        
        // The makeMove function from redis.ts now expects pgnMove and an optional promotion piece
        const updatedState = await makeMove(gameId, userId, pgnMove, promotion);
        ...
        const lastMove = updatedState.moves && updatedState.moves.length > 0 ? updatedState.moves[updatedState.moves.length - 1] : null;
       
        io.to(gameId).emit('move-made', {
          san: lastMove ? lastMove.san : pgnMove, // Send SAN of the move
          by: userId,
          color: updatedState.turn === 'white' ? 'black' : 'white', // The player who just moved
          promotion: lastMove ? lastMove.promotion : promotion || null,
          check: updatedState.check || false,
          gameState: updatedState // Ensure this includes board and clock
        });

        // Optionally, emit updated state separately for clarity (already done by gameState in move-made)
    ...
    // Check if a move requires promotion - This might need to be re-evaluated or removed
    // if client side chess.js instance can determine this before sending a move.
    // For now, let's assume client handles this. If it's kept, it should also work with PGN/SAN.
    socket.on('check-promotion', async ({ gameId, pgnMoveFrom, pgnMoveTo }) => { // Placeholder: needs actual PGN handling
      try {
        // This controller would need to be updated to understand PGN/SAN or from/to squares
        // const result = await checkPromotion(gameId, { pgnMoveFrom, pgnMoveTo }); 
        console.warn(`[${new Date().toISOString()}] 'check-promotion' event received for game ${gameId} but is not fully adapted for PGN. Client should handle promotion detection.`);
        // Simulate no promotion needed to avoid breaking clients expecting a response.
        socket.emit('promotion-not-required');
        /*
        if (result.needsPromotion) {
          socket.emit('promotion-required', {
            from: result.from, // These would be square notations
            to: result.to,
            color: result.color
          });
        } else {
